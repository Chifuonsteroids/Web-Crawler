...
def web_search_engine():
    ...
    try:
        connection = mysql.connector.connect(host=HOSTNAME, database=DATABASE, user=USERNAME, password=PASSWORD,autocommit=True)
        server_info = connection.get_server_info()
        print("MySQL connection is open on", server_info)
        while True:
...
add_url_to_frontier('https://www.thikawater.co.ke/')
if create_database():
    web_search_engine()
